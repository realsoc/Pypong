...
function processGetInfoFromGames1N(games,name){
var gamesInfos = {
...
function getInfoFromGames1N(callback,games,name,res){
	var gamesInfos = processGetInfoFromGames1N(games,name);
	callback(res,gamesInfos);
}
exports.getAllInfos = function(callback,res){
	async.parallel([
   	 	function(callback) {
        	Game.find({},function(err,games){
        		callback(null,games);
...
// optional callback
	function(err, results) {
		...
		for(i = 0;i<players.length;i++){
			retObj[players[i].name] = processGetInfoFromGames1N(games,players[i].name);
		}
		callback(res,retObj);
	})
}